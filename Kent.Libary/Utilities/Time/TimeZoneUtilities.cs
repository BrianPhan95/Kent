...
namespace Kent.Libary.Utilities.Time
{
    public static class TimeZoneUtilities
    {
        ...
        public static TimeSpan? ToUserTime(this TimeSpan? time, TimeZoneInfo timeZone)
        {
            ...
            // We add the date part to time for getting the right result in case daylight saving
            var localDate = DateTime.UtcNow.ToUserDateTime(timeZone).Date;
        ...
        public static TimeSpan ToUserTime(this TimeSpan time, TimeZoneInfo timeZone)
        {
            // We add the date part to time for getting the right result in case daylight saving
            var localDate = DateTime.UtcNow.ToUserDateTime(timeZone).Date;
            var dateTime = localDate.Add(time);
            var localDateTime = ToUserDateTime(dateTime, timeZone);
            return new TimeSpan(localDateTime.Hour, localDateTime.Minute, localDateTime.Second);
        }
