...
namespace Kent.Libary.Utilities
{
    public static class PasswordUtilities
    {
        ...
        {
            if (length < KentConfiguration.UniqueDateTimeFormat.Length + 8)
                length = KentConfiguration.UniqueDateTimeFormat.Length + 8;

            var randomKey = GetRandomString(length - KentConfiguration.UniqueDateTimeFormat.Length);

            var now = DateTime.UtcNow.ToString(KentConfiguration.UniqueDateTimeFormat);
