...
function registerPlugins() {
    ...
    */
    $(document).on('click', 'input[type="checkbox"].tri-state-check-box', function () {
        ...
        switch (el.data(state)) {
            ...
            case undefined:
                // Before the first click, the check-box can be either checked or unchecked
                if (el.prop('checked') == false) {
                    // It is un-checking, means before click it must be checked, go to indeterminate
                    el.data(state, indeterminate);
                    el.prop('indeterminate', true);
                    el.prop('checked', false);
                } else {
                    // It is checking, means before click it must be unchecked, go to checked
                    el.data(state, checked);
...
// Not allow click on tag a in content
$('.raw-content a').on('click', function (e) {
    e.preventDefault();
});

$(document).ready(function () {
    /*
     * Search & Clear button have same size
     */
    setTimeout(function () {
        var searchButtons = $('.btn-search');
        var searchLength = searchButtons.length;
        var maxWidth = 0;
        for (var i = 0; i < searchLength; i++) {
            if (maxWidth < $(searchButtons[i]).width()) {
                maxWidth = $(searchButtons[i]).width();
            }
        }

        // In case these button are not visible on UI, the maxWidth will be ZERO
        if (maxWidth == 0 && searchLength > 0) {
            var cloneEle = $(searchButtons[0]).clone();
            cloneEle.css('left', '-2000px');
            cloneEle.appendTo('body');
            maxWidth = cloneEle.width();
            cloneEle.remove();
            // Assume that search button is the first & clear button is second
            if (searchLength >= 2) {
                cloneEle = $(searchButtons[1]).clone();
                cloneEle.css('left', '-2000px');
                cloneEle.appendTo('body');
                var tmp = cloneEle.width();
                maxWidth = maxWidth < tmp ? tmp : maxWidth;
                cloneEle.remove();
            }
        }
        if (maxWidth > 0) {
            for (var k = 0; k < searchLength; k++) {
                $(searchButtons[k]).width(maxWidth);
            }
        }
